...
export function deformationWeight(point: Point2D, geometry: ZPlastyGeometryResult, params: ZPlastyParameters) {
  ...
  return Math.min(1, falloff * (1 - params.illustrativeFlexibility + params.deformationIntensity * 0.28))
}

export function illustrativeDeformationColor(weight: number, colorBlindSafe = false) {
  if (colorBlindSafe) {
